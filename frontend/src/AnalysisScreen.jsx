...
function BeatGrid({ profile }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="beatgrid" role="img" aria-label="شبكة الضربات">
      {/* منحنى الطاقة كتظليل خلفي خافت */}
      {energy.length > 1 && (
        <polyline
          points={energy.map((v, i) => `${x(i * eRes)},${H - 24 - v * 70}`).join(' ')}
          fill="none" stroke="rgba(152,162,168,0.35)" strokeWidth="2" strokeLinejoin="round"
        />
      )}
      {/* السلوتات: مستطيلات مانجو متوهجة */}
      {slots.map((s) => (
        <rect key={s.id}
          x={x(s.start)} y={38} width={Math.max(2, x(s.start + s.duration) - x(s.start))} height={64}
          fill="rgba(255,159,28,0.22)" stroke="rgba(255,159,28,0.9)" strokeWidth="1.5" rx="3"
        >
          <title>{`سلوت ${s.duration}ث — سعة ${s.capacity_syllables?.min}-${s.capacity_syllables?.max} مقاطع`}</title>
        </rect>
      ))}
      {/* خط الزمن */}
      <line x1={PAD} y1={H - 24} x2={W - PAD} y2={H - 24} stroke="#E8ECEE" strokeWidth="2.5" strokeLinecap="round" />
      {/* الضربات */}
      {beats.map((b, i) => (
        <line key={i}
          x1={x(b.t)} y1={b.type === 'downbeat' ? 30 : 52}
          x2={x(b.t)} y2={H - 24}
          stroke={b.type === 'downbeat' ? '#E8ECEE' : 'rgba(232,236,238,0.45)'}
      ...
      {beats.filter((b) => b.type === 'downbeat').map((b) => (
        <text key={b.bar} x={x(b.t)} y={22} textAnchor="middle"
          fontFamily="Space Mono" fontSize="11" fill="#A8E10C">{b.bar}</text>
      ))}
    </svg>
  )
}
